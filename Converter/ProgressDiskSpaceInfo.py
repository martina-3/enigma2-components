...
#				text = _("%s:Free:%s") % (entry[1], self.getSizeStr(list[2]))
				...
				text = ("%s: %s " + _("Free") + ": %s " + _("Used") + ": %s (%s%%)") % (entry[1], self.getSizeStr(list[0]), self.getSizeStr(list[2]), self.getSizeStr(list[1]), str(int(list[3])))
...
#				text = _("%s: %s Used: %s Free: %s") % (entry[1], self.getSizeStr(list[0]), self.getSizeStr(list[1]), self.getSizeStr(list[2]))
	...
	def getValue(self):
		...
		if self.type in (self.MEMTOTAL,self.MEMFREE,self.SWAPTOTAL,self.SWAPFREE):
			...
			path = {self.USBINFO: "/media/usb", self.MMCINFO: "/media/mmc", self.HDDINFO: "/media/hdd", self.FLASHINFO: "/"}[self.type]
